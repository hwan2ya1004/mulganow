...
def shop_prices():
    ...
    # ── 닭고기: 애드픽 재고에 생닭(육계) 자체가 없음 ────────────────────────
    # KAMIS "닭/육계(kg)"는 손질 전 생닭 소매가 기준인데, 애드픽 제휴몰
    # (11번가/SSG/컬리/GS SHOP/Hmall/CJ더마켓/롯데홈쇼핑) 재고를
    # "생닭"/"통닭"/"손질닭"/"냉장닭"/"닭고기" 등 다양한 검색어로 직접
    # 대조해본 결과, 매번 부위(가슴살/다리살/목살/어깨살/날개 등)나 조리
    # 상태(훈제/스테이크/술안주/튀김/양념치킨)만 다른 상품이 나올 뿐 생닭
    # 자체는 없었다(2026-07-21 확인). 부위/조리 키워드를 하나씩 막는 방식은
    # 표기 변형(예: "닭 다리살"처럼 띄어쓰기)이 계속 새로 뚫려 밑 빠진 독
    # 채우기가 되므로, 애초에 검색을 시도하지 않고 빈 결과를 반환한다.
    # generate_blog_post.py는 매칭 상품이 0개면 해당 품목을 글감에서
    # 제외하므로("살 곳이 없으면 글감으로 부적합"), 이 품목은 자동으로
    # 스킵된다.
